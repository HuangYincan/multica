  ...
  return {
    ...
    privacy: {
      ...
      intro: [
        ...
        "このポリシーは、お客様がご自身でホストする Multica には適用されません。そのデータはお客様のインフラにとどまり、当社が見ることはありません。セルフホストのサーバーが当社に送信するのは、1日1回の匿名スナップショットのみです。内容はサーバーのバージョンと、ワークスペース、メンバー、エージェント、タスクのおおよその数で、氏名、メールアドレス、コンテンツは含まれません。DO_NOT_TRACK=1 を設定すると送信を停止できます。",
      ...
      sections: [
        {
          ...
          bullets: [
            ...
            "利用状況とデバイスの情報: アプリのバージョン、OS、クライアントの種類、ランダムに生成されたインストール ID。ランタイムとして接続した各マシンの名前(デフォルトはホスト名)。クラッシュやエラーのレポート(送信前にメールアドレスやトークンを除去します)。",
        ...
        },
        {
          heading: "AI 機能",
          paragraphs: [
            "コーディングエージェントは、お客様自身のマシンやお客様が接続したランタイム上で動作し、Multica はそのタスクを調整します。Multica Cloud の一部の機能(チャットのタイトル生成や、次のアクションの提案など)では、結果を生成するために、関連するチャットメッセージを外部の大規模言語モデルプロバイダーに送信します。当社がお客様のコンテンツを AI モデルの学習に使うことはありません。",
          ],
        },
        {
          heading: "Cookie と分析",
        ...
        },
        {
          heading: "情報の共有先",
          paragraphs: [
            "当社が個人情報を共有するのは、Multica の運営を支援し、契約によって利用範囲が制限されたサービス提供者、または法令で求められる場合に限られます。主な共有先は次のとおりです。",
          ],
          bullets: [
        ...
        },
        {
          heading: "保存期間",
          paragraphs: [
            "アカウント情報とワークスペースのコンテンツは、アカウントやワークスペースが存在する間保存します。ワークスペースのオーナーがワークスペースを削除すると、その Issue やコメントなどのコンテンツは削除されます。サインインコードは10分で失効します。営業へのお問い合わせとフィードバックは、フォローアップと製品改善に必要な期間保存し、ご依頼があれば削除します。",
          ],
        },
        {
          heading: "お客様の選択と権利",
          paragraphs: [
            "お住まいの地域の法令によっては、個人情報へのアクセス、訂正、削除、書き出しを求める権利や、特定の処理に異議を唱えたり制限したりする権利がある場合があります。プロフィールは Multica 内でいつでも更新でき、オーナーであるワークスペースは設定から削除できます。アカウントの削除など、そのほかのご依頼は [support@multica.ai](mailto:support@multica.ai) までメールでご連絡ください。30日以内に回答します。",
          ],
        },
        {
          heading: "セキュリティ",
